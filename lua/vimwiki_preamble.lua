...
local function insert_markdown_preamble()
    ...
    if vim.bo.filetype == 'vimwiki' and vim.fn.expand('%:e') == 'md' then
        ...
        if vim.fn.getline(1) == '' then
            ...
            -- Insert the preamble
            vim.fn.setline(1, '---')
            vim.fn.append(1, 'title: ')
            vim.fn.append(2, 'author: darkelectron')
            vim.fn.append(3, 'date: '.. today ..'')
            vim.fn.append(4, 'updateddate: '.. today ..'')
            vim.fn.append(5, '---')
...
vim.api.nvim_create_user_command('InsertPreamble', insert_markdown_preamble, {})
vim.api.nvim_set_keymap('n', '<Leader>p', ':InsertPreamble<CR>', {noremap = true, silent = true})
